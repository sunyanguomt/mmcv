...
#include "ms_deform_attn_cuda_kernel.muh"

template <typename scalar_t>
void ms_deformable_im2col_cuda(musaStream_t stream, const scalar_t *data_value,
...
at::Tensor ms_deform_attn_cuda_forward(const at::Tensor &value,
  ...
  AT_ASSERTM(value.is_privateuseone(), "value must be a CUDA tensor");
  AT_ASSERTM(spatial_shapes.is_privateuseone(), "spatial_shapes must be a CUDA tensor");
  AT_ASSERTM(level_start_index.is_privateuseone(),
             "level_start_index must be a CUDA tensor");
  AT_ASSERTM(sampling_loc.is_privateuseone(), "sampling_loc must be a CUDA tensor");
  AT_ASSERTM(attn_weight.is_privateuseone(), "attn_weight must be a CUDA tensor");
...
void ms_deform_attn_cuda_backward(
  ...
  AT_ASSERTM(sampling_loc.is_privateuseone(), "sampling_loc must be a CUDA tensor");
  AT_ASSERTM(attn_weight.is_privateuseone(), "attn_weight must be a CUDA tensor");
  AT_ASSERTM(grad_output.is_privateuseone(), "grad_output must be a CUDA tensor");
