...
void TRTNMSCUDAKernelLauncher_float(const float* boxes, const float* scores,
  ...
  if (center_point_box == 1) {
    ...
    workspace = static_cast<char*>(workspace) +
                mmcv::getAlignedSize(num_batches * spatial_dimension * 4 *
                                     sizeof(float));
    thrust::transform(thrust::musa::par.on(stream), (NMSBox*)boxes,
  ...
  workspace = static_cast<char*>(workspace) +
              mmcv::getAlignedSize(spatial_dimension * sizeof(int));
  thrust::sequence(thrust::musa::par.on(stream), index_template,
  ...
  musaMemsetAsync(output_count, 0, sizeof(int), stream);

  // fill output with -1
  thrust::fill(thrust::musa::par.on(stream), output, output + output_length * 3,
               -1);
  cudaCheckError();
  ...
  for (int batch_id = 0; batch_id < num_batches; ++batch_id) {
    for (int cls_id = 0; cls_id < num_classes; ++cls_id) {
      ...
      // sort boxes by score
      musaMemcpyAsync(scores_sorted, scores + batch_cls_id * spatial_dimension,
                      spatial_dimension * sizeof(float),
                      musaMemcpyDeviceToDevice, stream);
      cudaCheckError();

      musaMemcpyAsync(index_cache, index_template,
                      spatial_dimension * sizeof(int), musaMemcpyDeviceToDevice,
                      stream);
      cudaCheckError();

      thrust::sort_by_key(thrust::musa::par.on(stream), scores_sorted,
                          scores_sorted + spatial_dimension, index_cache,
                          thrust::greater<float>());

      if (center_point_box == 1) {
        thrust::gather(thrust::musa::par.on(stream), index_cache,
      ...
      if (score_threshold > 0.0f) {
        thrust::transform_if(
            thrust::musa::par.on(stream), (NMSBox*)boxes_sorted,
